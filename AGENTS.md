...
- 利用者に見える機能追加・仕様変更・挙動変更・リリース単位の修正を行う場合に、バージョン更新を検討する。
- 文書のみの更新、作業メモ整理、テスト補助、内部説明の追記など、利用者向けの挙動に影響しない変更では、原則としてバージョンを更新しない。
- 採番形式は `MAJOR.MINOR.PATCH-beta.N` とする。
- ベータ版の継続改善中に、同一機能群の反復調整を行う場合は `beta.N` を増やす。
- 機能追加や利用者に見える改善をひと区切りとして出す場合は `MINOR` を上げ、`beta.N` は `1` から振り直す。
- 緊急の小修正だけを単独で出す場合のみ `PATCH` を上げる。
- 正式版へ移る場合は、同じ系列の `-beta.N` を外す。
- `package.json` の `version` を更新した場合は、表示用の版番号や関連文書（必要なら `index.html`、`README.md`、`EXPLANATION.md`）も整合させる。
- `package.json` を変更していないのに `package-lock.json` の版情報だけが変わった場合は、更新せず先に理由を確認する。
